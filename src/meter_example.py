...
# This legacy file teaches you how to draw a canvas with lots of things together
...
class DummyValueMonitor(object):
  def get(self):
...
class MeterEventHandler(object):
  ...
  def draw(self, cr):
    ...
    if (self.right_fill_path == None):
      cr.new_path();
      cr.move_to(width / 2 + padding, height);
      cr.arc_negative(width / 2, height / 2, radius, math.pi / 2 - math.asin(float(padding) / radius), math.asin(float(radius - padding * 2) / radius));
      cr.line_to(width, height / 2 + radius - padding * 2);
